// 배열과 메서드 문제 3 <특정 범위에 속하는 요소 찾기(배열 변경하기)>
...
1. let arr = [5, 3, 8, 1];
2.
3. filterRangeInPlace(arr, 1, 4); // 1과 4 사이에 있지 않은 요소는 모두 제거함
6.
7. alert( arr );  // [3, 1]
...
function filterRangeInPlace(array, a, b){
    array.forEach((item, index, array) => {(item >= a && item <= b) ? true : array.splice(index, 1)});
}
